...
(function() {
	window.RTCMultiConnection = function(channel, extras) {
			...
			if (!self.openSignalingChannel) {
				...
				self.openSignalingChannel = function(config) {
					...
					var socket = new window.Firebase('https://'
							+ (self.firebase || 'chat') + '.firebaseIO.com/'
							+ channel);
					...
					if (channel != self.channel
							|| (self.isInitiator && channel == self.channel))
		...
		function init() {
			...
			self.config = {
				...
				autoCloseEntireSession : typeof self.autoCloseEntireSession == 'undefined' ? false
						: self.autoCloseEntireSession,
				connection : self
			};
			rtcSession = new RTCMultiSession(self.config);

			// bug: these two must be fixed. Must be able to receive many files
			// concurrently.
		...
		function captureUserMedia(callback) {
			...
			var mediaElement = document
					.createElement(session.isAudio() ? 'audio' : 'video');
			var mediaConfig = {
				video : mediaElement,
	...
	var RTCPeerConnection = function(options) {
		var w = window, PeerConnection = w.mozRTCPeerConnection
				|| w.webkitRTCPeerConnection, SessionDescription = w.mozRTCSessionDescription
				|| w.RTCSessionDescription, IceCandidate = w.mozRTCIceCandidate
				|| w.RTCIceCandidate;
		...
		function _openOffererChannel() {
			channel = peerConnection.createDataChannel(options.channel
					|| 'RTCDataChannel', moz ? {} : {
				reliable : false
			});
	...
	function RTCMultiSession(config) {
		var session = config.session || Session.AudioVideo, direction = config.direction
				|| Direction.ManyToMany, self = {
			id : uniqueToken(),
			sessionid : config.channel,
			sockets : [],
			socketObjects : {}
		}, channels = '--', isbroadcaster, isAcceptNewSession = true, defaultSocket = {}, RTCDataChannels = [];
		...
		function onDefaultSocketResponse(response) {
			...
			if (response.newParticipant && self.joinedARoom
					&& self.broadcasterid === response.userid)
				onNewParticipant(response.newParticipant, response.extra);

			if (response.userid && response.targetUser == self.id
					&& response.participant
					&& channels.indexOf(response.userid) == -1) {
		...
		function openSubSocket(_config) {
			...
			var socket = config.openSignalingChannel(socketConfig), isofferer = _config.isofferer, extra = _config.extra
					|| {}, isGotRemoteStream, inner = {}, mediaElement = document
					.createElement(session.isAudio() ? 'audio' : 'video'), peer;
			...
			function initPeer(offerSDP) {
				if (direction === Direction.OneToOne
						&& window.isFirstConnectionOpened)
					return;
				if (!offerSDP) {
				...
				if (session.isAudio()) {
					/*
					 * OfferToReceiveVideo MUST be false for audio-only
					 * streaming
					 */
			...
			function onChannelOpened(channel) {
				...
				if (config.onChannelOpened)
					config.onChannelOpened(extra);
				onSessionOpened();
			}

			function onSessionOpened() {
				if (isGotRemoteStream)
					return;
				window.isFirstConnectionOpened = isGotRemoteStream = true;

				if (direction === Direction.ManyToMany && isbroadcaster
						&& channels.split('--').length > 3) {
			...
			function sendsdp(sdp) {
				...
				var firstPart = sdp.slice(0, part), secondPart = sdp.slice(
						part, sdp.length - 1), thirdPart = '';
			...
			function socketResponse(response) {
				...
				if (response.firstPart || response.secondPart
					...
					if (response.firstPart) {
						// sdp sender's user id passed over onRemoteStream and
						// onopen
				...
				if (response.left) {
					...
					if (response.closeEntireSession) {
						...
						if (self.sockets[_config.socketIndex])
							delete self.sockets[_config.socketIndex];
					...
					if (config.onleave)
						config.onleave(response.id, response.extra);
				}
		...
		function leaveARoom(channel) {
			...
			if (isbroadcaster) {
				if (config.autoCloseEntireSession)
			...
			if (!channel) {
				// closing all sockets
				var sockets = self.sockets, length = sockets.length;
		...
		return {
			...
			joinSession : function(_config, extra) {
				...
				// used to make sure each room's messages must be stay in the
				// same room
			...
			leave : function(userid, autoCloseEntireSession) {
				if (autoCloseEntireSession)
					config.autoCloseEntireSession = true;
				leaveARoom(userid);
				if (!userid) {
	...
	var FileSender = {
		send : function(config) {
			...
			var packetSize = 1000 /* chars */, textToTransfer = '', numberOfPackets = 0, packets = 0;
	...
	var TextSender = {
		send : function(config) {
			var channel = config.channel, initialText = config.text, packetSize = 1000 /* chars */, textToTransfer = '';
