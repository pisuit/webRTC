var socket = new WebSocket('ws://172.16.72.137:1337/'); // change the IP address
														// to your websocket
														// server
...
function createPeerConnection() {
	try {
		...
		try {
			peerConn = new RTCPeerConnection('STUN ' + stunServer,
					onIceCandidate00);
	...
	if (msg.type === 'offer') {
		...
		if (!started && localStream) {
			...
			if (isRTCPeerConnection) {
				...
				peerConn.createAnswer(setLocalAndSendMessage, null,
						mediaConstraints);
			} else {
				// set remote description
				peerConn.setRemoteDescription(peerConn.SDP_OFFER,
						new SessionDescription(msg.sdp));
				// create answer
				var offer = peerConn.remoteDescription;
				var answer = peerConn.createAnswer(offer.toSdp(),
						mediaConstraints);
				console.log("Sending answer to peer.");
...
//	var msg = JSON.parse(message);
//
//	// if (msg.type === 'offer') --> will never happened since
//	// isRTCPeerConnection=true initially
//	if (msg.type === 'answer' && started) {
//		peerConn.setRemoteDescription(peerConn.SDP_ANSWER,
//				new SessionDescription(msg.sdp));
//	} else if (msg.type === 'candidate' && started) {
...
function startVideo() {
	...
	try {
		...
		navigator.webkitGetUserMedia("video,audio", successCallback,
				errorCallback);
	}
